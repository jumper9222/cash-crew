...
import { getFriends } from "../features/friends/friendsSelector";
import { useAppDispatch, useAppSelector } from "../hooks/useReduxHooks";

export default function FriendsPage() {
  const dispatch = useAppDispatch();
  const { uid } = useAppSelector((state) => state.currentUser);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    dispatch(addFriend({ email: query, currentUserUid: uid }))
  ...
  return (
    <Container>
      ...
      <ListGroup className="mt-3 flex-grow-1 overflow-auto">
              ...
              <ListGroupItem key={key}>
                <Row>
                  ...
                  >
                    <p className="m-0">
                      <strong>
                        {friend.displayName ? friend.displayName : friend.email}
                      </strong>
                    </p>
                    <p className="m-0 text-secondary">{friend.email}</p>
                  </Col>
                </Row>
              </ListGroupItem>
            );
