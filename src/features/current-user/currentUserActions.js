    ...
    async ({ currentUser, profilePicture, displayName, birthday, gender }) => {
        ...
        if (profilePicture) {
            ...
            await uploadBytes(storageRef, profilePicture)
                ...
                .then((url) => {
                    photoURL = url
                    console.log(`Profile picture uploaded to firestore storage successfully.`)
                })
                .catch(error => console.error('Error uploading new profile picture: ', error.message))
        }
        ...
        await updateProfile(auth.currentUser, { photoURL, displayName })
            ...
            .catch((error) => console.error(`Something went wrong when trying to update user in firebase auth: ${error.message}`))
        ...
        await setDoc(userRef, newProfileDetails, { merge: true })
            ...
            .catch((error) => console.error(`Something went wrong when trying to update user database: ${error.message}`))
